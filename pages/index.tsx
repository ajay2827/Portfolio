...
const index=()=> {
  ...
  return (
    <motion.div className='flex flex-col flex-grow px-6 pt-1' variants={routeAnimation} initial='initial' animate='animate' exit='exit'>
      <h5 className='my-3 font-semibold text-white dark:text-gray-600 font-Hanken'>I am currently pursuing B.Tech Degree (2nd year) in Computer Science Engineering from IIITM Gwalior.<br/>I am Full Stack Developer based in India and build dynamic data-driven websites and applications using a rich ensemble of technologies and techniques.</h5>
      
      <div className='flex-grow p-4 mt-4 dark:bg-gray-800 bg-dark-100 ' style={{marginLeft:'-1.5rem',marginRight:'-1.5rem'}}>
        ...
        <motion.div variants={stragger} initial='initial' animate='animate' className='grid gap-6 mt-5 lg:grid-cols-2'>
          {
            services.map(service=>( <motion.div variants={FadInup} key={service.id} className='rounded-lg dark:bg-gray-700 bg-dark-200 lg:col-span-1'>
